...
if [ ! -f "$INSTALL_DIR/nginx/ssl/server.crt" ]; then

openssl req -x509 -nodes -days 365 \
-newkey rsa:2048 \
-keyout "$INSTALL_DIR/nginx/ssl/server.key" \
-out "$INSTALL_DIR/nginx/ssl/server.crt" \
-subj "/C=US/ST=None/L=None/O=ServerKit/CN=localhost"
...
# nginx site config
cat > /etc/nginx/sites-available/serverkit.conf << 'EOF'
server {
    listen 80;
